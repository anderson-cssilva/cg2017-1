...
#include <GL/glut.h>
#include <stdlib.h>
#include "renderingEllipse.cpp"

/*Definição da superfície*/
int N = 170;
GLfloat x_min = 0.0, x_max = 2*(3.14159265359), y_min = 0.0, y_max = (3.14159265359), R = 30.0;
vector<vector<GLCell> > malha;
Rendering renObj(N, x_min, x_max, y_min, y_max, R);
	...
	for (int i = 1; i < (N+2) - 1; i++) {
		for (int j = 1; j < (N+2) - 1; j++) {
			...
			glBegin(GL_QUAD_STRIP);
				...
				glNormal3f(malha[i+1][j+1].vetor_normal.x, malha[i+1][j+1].vetor_normal.y, malha[i+1][j+1].vetor_normal.z);
...
void Teclado (unsigned char key, int x, int y)  
{
	switch(key)
	{
		...
		case 'B':	if (cores[emite][2]<1.0) cores[emite][2]+=0.1;
					break;
		case 'a':	posicaoLuz[0]-=10;
					break;
		case 'd':	posicaoLuz[0]+=10;
					break;
		case 'w':	posicaoLuz[1]+=10;
					break;
		case 's':	posicaoLuz[1]-=10;
	}
	glutPostRedisplay();
}
